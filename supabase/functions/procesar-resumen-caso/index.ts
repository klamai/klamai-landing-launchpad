...
serve(async (req)=>{
  ...
  try {
    ...
    const { caso_id, resumen_caso, transcripcion_chat, nombre_borrador, motivo_consulta: motivo_inicial, files: filesValue } = payload;
    ...
    log('info', 'Payload recibido y procesado', {
      caso_id,
    ...
    const promesas = [
      ...
      ejecutarAsistente(Deno.env.get("ASISTENTE_PROPUESTAS_ID"), prompt_propuesta),
    ...
    const [guiaResult, propuestaResult, clasificacionResult, ...archivosResults] = await Promise.allSettled(promesas);
    ...
    const propuesta_raw = propuestaResult.value;
    ...
    log('info', 'Tareas en paralelo completadas', {
      caso_id,
    ...
    return new Response(JSON.stringify({
      success: true,
      message: "El caso ha sido procesado."
    }), {
      headers: {
        ...corsHeaders,
