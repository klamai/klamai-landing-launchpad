  ...
  try {
    ...
    switch (event.type) {
      ...
      default:
        ...
        processingResult = { success: true, message: "Event type not handled but acknowledged" };
    }
    ...
    logStep("Event processing completed", { success: processingResult.success });
...
async function handleCheckoutCompleted(event: any, supabase: any) {
  ...
  if (!casoId) {
    return { success: false, message: "No caso_id in session metadata" };
  }
  ...
  try {
    ...
    logStep("Checkout completed successfully", { casoId });
    return { success: true, message: "Case updated and payment recorded" };

  } catch (error) {
    const errorMessage = error instanceof Error ? error.message : String(error);
    logStep("Error processing checkout completion", { error: errorMessage });
...
async function handlePaymentSucceeded(event: any, supabase: any) {
  ...
  try {
    ...
    if (error) {
      logStep("Payment update failed", { error });
    }
