...
serve(async (req) => {
  ...
  try {
    ...
    const session = await stripe.checkout.sessions.create({
      ...
      line_items: [
        {
          price_data: {
            currency: 'eur',
            product_data: {
              name: 'Consulta Legal Individual - klamAI',
              description: 'Análisis completo de tu caso legal con propuesta personalizada',
            },
      ...
      success_url: `${req.headers.get('origin')}/chat?caso_id=${casoId}&payment=success`,
