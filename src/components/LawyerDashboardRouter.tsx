...
const LawyerDashboardRouter = ({ children }: LawyerDashboardRouterProps) => {
  ...
  if (userRole !== 'abogado') {
    console.log('🚫 Acceso denegado: usuario no es abogado, rol:', userRole);
    return <Navigate to="/auth" replace />;
  }

  // If user is a lawyer, pass the children with role context
  console.log('✅ Acceso autorizado para abogado tipo:', lawyerType);
  return <>{children}</>;
};
