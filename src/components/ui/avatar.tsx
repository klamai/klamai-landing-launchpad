...
const avatarStatusVariants = cva('flex items-center rounded-full w-2 h-2 border-2 border-background', {
  variants: {
    variant: {
      online: 'bg-green-600',
...
function Avatar({ className, ...props }: React.ComponentProps<typeof AvatarPrimitive.Root>) {
  return (
    <AvatarPrimitive.Root data-slot="avatar" className={cn('relative flex shrink-0 w-10 h-10', className)} {...props} />
  );
}
...
function AvatarIndicator({ className, ...props }: React.HTMLAttributes<HTMLDivElement>) {
  return (
    <div
      ...
      className={cn('absolute flex w-6 h-6 items-center justify-center', className)}
      {...props}
    />
  );
}
