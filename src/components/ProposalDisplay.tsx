...
const ProposalDisplay = ({ proposalData, casoId, isModal = false, onClose }: ProposalDisplayProps) => {
  ...
  const handleBudgetRequest = async (data: BudgetRequestFormData) => {
    try {
      ...
      // 3. Marcar caso como listo para propuesta Y solicitud de presupuesto
      await supabase.rpc('set_caso_listo_para_propuesta', { p_caso_id: casoId });
      
      // 3.1. Marcar como solicitud de presupuesto (esto disparará el trigger automáticamente)
      await supabase
        .from('casos')
        .update({ solicitud_presupuesto_whatsapp: true } as any)
        .eq('id', casoId as any);
